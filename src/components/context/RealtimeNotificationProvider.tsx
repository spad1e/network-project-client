...
export const RealtimeNotificationContext = createContext<
  ...
  const addNotification = (c: INotification) => {
    setNotification((prev) => {
      const exists = prev.find(
        (item) => item.id === c.id && item.type === c.type,
      );
      if (exists) {
        return prev;
      }
      if (currChatRef.current?.type === c.type) {
        if (
          (c.type == "group" && currChatRef.current?.id === c.groupId) ||
          (c.type == "direct" && currChatRef.current?.id === c.sender)
        ) {
          return prev;
        }
      }
      if (c.groupId) {
        map_notificationRef.current.set(
          JSON.stringify({ id: c.groupId, type: c.type }),
          c,
        );
      } else {
        map_notificationRef.current.set(
          JSON.stringify({ id: c.sender, type: c.type }),
          c,
        );
      }
      return [...prev, c];
    ...
    setTimeout(() => {
      setNotification((prev) =>
        prev.filter((item) => item.id !== c.id || item.type !== c.type),
      );
